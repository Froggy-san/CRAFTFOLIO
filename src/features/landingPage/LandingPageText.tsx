...
const LandingPageText = ({
  ...
  return (
    <div
      style={style}
      className={`z-10 flex h-full max-w-[1200px] flex-1 flex-col gap-6 overflow-hidden whitespace-pre-wrap text-center text-xl xs:gap-10 sm:text-left ${className}`}
    >
      {/* className="  text-xl  xs:text-1xl md:text-2xl lg:text-4xl 3xl:text-5xl " */}
      {landingPageText.primaryText && (
        <h1 className="break-all text-[clamp(1.25rem,3.3vw,2.7rem)] leading-[clamp(1.7rem,4vw,3rem)]">
          {landingPageText.primaryText}
        </h1>
      )}
      {/* text-xs xs:text-sm sm:text-xl  lg:text-2xl  */}
      {landingPageText.secondaryText && (
        <h2 className="text-[clamp(0.75rem,2.2vw,1.8rem)] leading-[clamp(1rem,3.5vw,2.2rem)] 3xl:text-3xl">
          {landingPageText.secondaryText}
        </h2>
      )}
